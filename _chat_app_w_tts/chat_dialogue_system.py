...
# Add parent directory to path to import TTS system
...
class VoiceChatSystem:
    def __init__(self):
        ...
        keyboard.on_press_key('space', self._handle_interrupt, suppress=False)
        
    def _handle_interrupt(self, event):
        if keyboard.is_pressed('ctrl'):
            print("Interrupt triggered!")
            self.interrupt_current_speech()
            # Only suppress the key if Ctrl is pressed
            return False
        # Don't suppress the key if Ctrl is not pressed
        return True
